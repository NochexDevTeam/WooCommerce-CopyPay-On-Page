...
/**
 * Product-page Google Pay express checkout (isolated from checkout Google Pay flow).
 */
trait NCX_CP_API_Product_Google_Pay {

    // Order meta value tagging product-page Google Pay orders (traits cannot declare constants on PHP < 8.2).
    private static $product_google_pay_source = 'google_pay_product';

    // Wires product-page Google Pay hooks without touching checkout handlers.
    public function register_product_google_pay_hooks(): void {
        add_action('woocommerce_after_add_to_cart_button', [$this, 'render_product_google_pay_markup'], 15);
        add_action('wp_enqueue_scripts', [$this, 'maybe_enqueue_product_google_pay_assets'], 25);
    }

    // Returns true when the admin product-page Google Pay setting is enabled.
    public function is_product_google_pay_enabled(): bool {
        return $this->is_google_pay_enabled()
            && 'yes' === $this->get_option('enable_google_pay_product_page', 'no');
    }

    // Returns true when the product-page Google Pay UI should render for the current request.
    public function should_render_product_google_pay_ui(?WC_Product $product = null): bool {
        if (!$this->is_product_google_pay_enabled() || 'yes' !== $this->enabled) {
            return false;
        }
        ...
        if (!$this->customer_profile_ready_for_product_google_pay($product)) {
            return false;
        }

        if (!$product instanceof WC_Product) {
            $product = $this->get_current_single_product_for_google_pay();
        }

        return $this->is_product_eligible_for_google_pay($product);
    }

    // Returns true when the logged-in customer has required billing (and shipping when needed).
    public function customer_profile_ready_for_product_google_pay(?WC_Product $product = null): bool {
        $validation = $this->validate_customer_profile_for_product_google_pay($product);
     ...
     */
    private function validate_customer_profile_for_product_google_pay(?WC_Product $product = null) {
        if (!is_user_logged_in()) {
            return new WP_Error(
                'ncx_cp_product_gp_guest',
                __('Please log in to use Google Pay on the product page.', 'ncx-cp-api')
            );
        }

        if (!function_exists('WC') || !WC()->customer) {
            return new WP_Error(
                'ncx_cp_product_gp_no_customer',
        ...
        $posted         = $this->get_customer_profile_as_checkout_post_for_google_pay($needs_shipping);
     ...
     */
    private function get_customer_profile_as_checkout_post_for_google_pay(bool $needs_shipping): array {
        ...
        $posted['ship_to_different_address'] = $this->customer_has_distinct_shipping_address_for_google_pay() ? '1' : '0';

        return $posted;
    }

    // Returns true when the customer shipping address differs from billing.
    private function customer_has_distinct_shipping_address_for_google_pay(): bool {
        if (!function_exists('WC') || !WC()->customer) {
            return false;
        }
    ...
    // Returns true when this product type can be bought via product-page Google Pay.
    private function is_product_eligible_for_google_pay(?WC_Product $product): bool {
        if (!$product instanceof WC_Product) {
            return false;
        }
    ...
    private function get_current_single_product_for_google_pay(): ?WC_Product {
    ...
    // Outputs the product-page Google Pay mount containers below add to cart.
    public function render_product_google_pay_markup(): void {
        if (!is_product() || !$this->should_render_product_google_pay_ui()) {
            return;
        }

        echo '<div id="ncx-cp-product-googlepay-wrap" class="ncx-cp-product-googlepay-wrap">';
        echo '<div id="ncx-cp-product-googlepay-trigger" class="ncx-cp-googlepay-frame"></div>';
        echo '<div id="ncx-cp-product-googlepay-pay" class="ncx-cp-googlepay-pay"></div>';
        echo '</div>';
    }

    // Enqueues product-page Google Pay assets on single product pages only.
    public function maybe_enqueue_product_google_pay_assets(): void {
        if (!is_product() || !$this->is_product_google_pay_enabled() || 'yes' !== $this->enabled) {
            return;
        }

        $product = $this->get_current_single_product_for_google_pay();
        if (!$this->should_render_product_google_pay_ui($product)) {
            return;
        }
        ...
        $handle = 'ncx-cp-api-product-googlepay';
        wp_enqueue_script(
            $handle,
            plugins_url('../assets/js/product-googlepay.js', __FILE__),
            ['jquery'],
            NCX_CP_API::asset_version('assets/js/product-googlepay.js'),
            true
        );

        wp_localize_script($handle, 'ncxCpProductGooglePay', $this->get_product_google_pay_script_config($product));
     ...
     */
    public function get_product_google_pay_script_config(?WC_Product $product = null): array {
        if (!$product instanceof WC_Product) {
            $product = $this->get_current_single_product_for_google_pay();
        }
        ...
        return [
            ...
            'ajaxUrl'               => WC_AJAX::get_endpoint('ncx_cp_googlepay_product_process'),
            'checkoutRedirectUrl'   => WC_AJAX::get_endpoint('ncx_cp_googlepay_product_checkout_redirect'),
            ...
            'googlePay'             => $this->get_product_page_google_pay_client_config($product),
    ...
    /**
     * Google Pay client config for product pages (separate process URL and shipping flags).
     *
     * @return array<string, mixed>
     */
    private function get_product_page_google_pay_client_config(?WC_Product $product = null): array {
        $config = $this->get_google_pay_client_config();
        $config['processUrl'] = WC_AJAX::get_endpoint('ncx_cp_googlepay_product_process');
    ...
    /**
     * AJAX: logged-in shoppers with complete profiles — buy now + Google Pay session.
     */
    public function ajax_googlepay_product_process(): void {
        $this->googlepay_log('product process: request received', [
            'logged_in' => is_user_logged_in(),
        ]);

        if (!is_user_logged_in()) {
            wp_send_json_error([
                'message'  => __('Please log in to use Google Pay.', 'ncx-cp-api'),
        ...
        if (!$this->is_product_google_pay_enabled()) {
            wp_send_json_error(['message' => __('Google Pay on the product page is not enabled.', 'ncx-cp-api')]);
            return;
        }

        $product = $this->load_product_from_request_for_google_pay();
        if (is_wp_error($product)) {
            wp_send_json_error(['message' => $product->get_error_message()]);
            return;
        }

        $profile = $this->validate_customer_profile_for_product_google_pay($product);
        ...
        $cart_result = $this->build_product_buy_now_cart_from_request_for_google_pay($product);
        ...
        $session_apply = $this->apply_customer_profile_to_session_for_product_google_pay();
        ...
        $order->update_meta_data('_ncx_cp_payment_source', self::$product_google_pay_source);
        $order->set_payment_method_title(__('Google Pay', 'ncx-cp-api'));
        if (!$was_reused) {
            $order->add_order_note(__('Product-page Google Pay payment initiated.', 'ncx-cp-api'));
        }
        $order->save();
        ...
        $response = $this->create_product_google_pay_session_response($order, $use_test);
     ...
     */
    public function ajax_googlepay_product_checkout_redirect(): void {
        if (!is_user_logged_in()) {
            wp_send_json_error([
     ...
     */
    private function load_product_from_request_for_google_pay() {
        ...
        if (!$product instanceof WC_Product || !$this->is_product_eligible_for_google_pay($product)) {
            return new WP_Error('ncx_cp_product_invalid', __('This product cannot be purchased with Google Pay.', 'ncx-cp-api'));
        }
     ...
     * @return true|WP_Error
     */
    private function build_product_buy_now_cart_from_request_for_google_pay(WC_Product $product) {
        ...
        $variations   = $this->parse_variation_attributes_from_request_for_google_pay();

        if ($product->is_type('variable')) {
            if ($variation_id <= 0) {
                return new WP_Error(
                    'ncx_cp_variation_required',
                    __('Please choose product options before using Google Pay.', 'ncx-cp-api')
                );
            }
     ...
     * @return array<string, string>
     */
    private function parse_variation_attributes_from_request_for_google_pay(): array {
        $attributes = [];
     ...
     * @return true|WP_Error
     */
    private function apply_customer_profile_to_session_for_product_google_pay() {
        if (!function_exists('WC') || !WC()->customer || !WC()->session) {
            return true;
        }

        $posted = $this->get_customer_profile_as_checkout_post_for_google_pay(
     ...
     */
    private function create_product_google_pay_session_response(WC_Order $order, bool $use_test) {
        $prepared = $this->create_committed_provider_checkout($order, 'googlepay');
